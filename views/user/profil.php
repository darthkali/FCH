...
<div class="Content" id="fadeIn">
    <form autocomplete= "off" action="?c=user&a=profil<?=$userInformation?>" method="post" id="formProfil" enctype="multipart/form-data" >
        ...
        <? if($userRole == role::ADMIN){?>
            ...
            <? } ?>
            <div class="input">
                <label for="functionClubProfil">Funktion im Verein:</label><br>
                <select name="functionClubProfil" id="functionClubProfil">
                    <? foreach ($allFunctions as $function) { ?>
                    <option value= <?=$function['ID']?> <?=($userFunction === $function['ID']) ? 'selected' : ''?> ><?=$function['NAME']?></option>
                    <? } ?>
                </select>
                <span class="error-message" id="errorFunctionClubProfil"></span>
            </div>
        <? } ?>
